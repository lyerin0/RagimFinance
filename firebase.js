...
const FB = {
  ...
  listen(){
    ...
    onSnapshot(doc(this.db,'world','rate'), snap => {
      const d = snap.data();
      if(!d) return;
      if(typeof d.kr === 'number') RATE.KR = d.kr;
      if(typeof d.us === 'number') RATE.US = d.us;
      if(typeof d.v  === 'number') RATE.KR = d.v;   // 예전 형식
      renderRate();
    });
  ...
  async setRate(kr, us){
    if(!this.on) return;
    await setDoc(doc(this.db,'world','rate'), { kr, us, by:this.uid, at:Date.now() })
